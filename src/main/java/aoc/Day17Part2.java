...
//         task();
    ...
    private static void calculateCycleInfo() {
        int allSteps = 10000;
        int cycleStart = 1220;
        int initialScore = 1912;
        int cycleSize = 1740;
        int cycleScore = 2716;
        int stepsWithoutInit = allSteps - cycleStart;
        int totalCycles = stepsWithoutInit / cycleSize;

        int totalWithoutRemaining = totalCycles * cycleScore + initialScore;

        int remainingToCalc = stepsWithoutInit - (totalCycles * cycleSize);
        int scoreForSteps = 123;
        int scoreFor10000 = 116;
        int scoreFor20000 = 2103;
        int scoreFor15000 = 2493;

        int total = totalWithoutRemaining + scoreFor15000;
    ...
    private static void task() throws IOException {
        ...
        int cycleIter = 0;
        boolean startCycleCOunter = false;
        ...
        for (int step = 0; step < 10001; step++) {
            ...
            if (startCycleCOunter) {
                cycleIter++;
//                if ((height - 1903) % 2715 == 0) {
                if ((step - 1215) % 1740 == 0) {
                    height = calculateHeight(floor);
...
//                }
    ...
    private static Integer moveShape(String motions, Set<Coordinate> floor, int absoluteCountr, Shape shape,
                                     int floorRow, int step) {
